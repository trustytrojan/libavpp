...
public:
	...
	void print_info(int index) { av_dump_format(_fmtctx, index, _fmtctx->url, 0); }
	 ...
	 * @returns The string value associated with `key`, or `nullptr` if no entry exists for `key`.
	 */
	const char *metadata(const char *const key, const AVDictionaryEntry *prev = NULL, const int flags = 0) const
	{
		const auto *const entry = av_dict_get(_fmtctx->metadata, key, prev, flags);
	...
	Stream find_best_stream(
		const AVMediaType type, int wanted_stream_nb = -1, int related_stream = -1, const AVCodec **decoder_ret = NULL, int flags = 0) const
	{
