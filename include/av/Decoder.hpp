...
public:
	...
	~Decoder() { av_frame_free(&_frm); }

	/**
	 * Sends a packet to the decoder.
	 * @param packet can be `NULL` (or an `AVPacket` with data set to `NULL` and
	 * size set to `0`); in this case, it is considered a flush packet, which
	 * signals the end of the stream.
	 * @return whether the send was successful
	 * @throws `av::Error` if any `avcodec_*` functions fail
	 * @note `true` is also returned in the case that the decoder has output
	 * frames to return to the caller.
	 */
	 ...
	 * @retval `NULL` if either the codec requires a new packet to be sent, or
	 * the codec has been fully flushed.
	 * @throws `av::Error` if any `avcodec_*` functions fail
